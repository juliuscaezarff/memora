...
export default function Bookmarks({
	...
	const { data: labels = [] } = useQuery(orpc.label.getAll.queryOptions());
			...
			{isEmpty ? (
				<main className="flex flex-1 items-center justify-center px-4 py-12">
					<section className="flex w-full max-w-md flex-col items-center rounded-lg border border-[#262626] bg-black px-6 py-10 text-center sm:px-10">
						...
						<h1 className="mt-3 text-balance font-semibold text-[#ededed] text-sm leading-tight">
							No folders yet
						</h1>
						<p className="mt-2 max-w-xs text-pretty text-[#888] text-[13px] leading-5">
						...
						<Button
							...
							className="mt-4 rounded-md bg-[#ededed] px-3 text-[#0a0a0a] text-[13px] hover:bg-[#d4d4d4]"
						>
							Create folder
						</Button>
					</section>
